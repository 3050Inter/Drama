...
export default function LaborPage({date,onChanged}:{date:string;onChanged:()=>Promise<void>}){
  ...
  return <div className="space-y-4">
    ...
    <div><div className="mb-2 text-lg font-black">오늘 인건비 내역</div>{!rows.length&&<div className="rounded-[24px] bg-[#111A2E] p-6 text-center text-sm text-slate-400">인건비 내역이 없습니다.</div>}<div className="space-y-3">{rows.map(item=><div key={item.id} className="rounded-[24px] bg-[#111A2E] p-4"><div className="flex items-start justify-between gap-4"><div><div className="font-black">{item.employee}</div><div className="mt-1 text-sm text-slate-300">테이블 {item.tableNo||"-"} / 지급금액 {money(Number(item.tc||0)+Number(item.dailyPay??0))}</div>{item.memo&&<div className="mt-1 text-sm text-slate-400">📝 {item.memo}</div>}</div><div className="text-right"><button onClick={()=>remove(item.id)} className="inline-flex items-center gap-1 text-xs text-red-400"><Trash2 size={14}/>삭제</button></div></div></div>)}</div></div>
  </div>;
}
