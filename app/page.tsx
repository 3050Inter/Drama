...
export default function Page() {
  ...
  const saveTransaction = async (data: Parameters<typeof addTransaction>[0]) => {
    ...
    const row: Transaction = { ...data, id: res.id || `LOCAL-${Date.now()}` };
    if (row.date === date) {
  ...
  const saveEdit = async (id: string, data: Parameters<typeof updateTransaction>[1]) => {
    ...
    if (oldRow) {
      setDashboard((prev) => adjustDashboard(adjustDashboard(prev, oldRow, -1), nextRow, 1));
    } else {
      await reload();
    }
  };
