...
export const getHome = (date: string) => getJson<ApiResult<{ dashboard: Dashboard; rows: Transaction[] }>>("home", { date });
...
export const addLabor = (data: { date: string; employee: string; tableNo: string; tc: number; memo: string }) => postJson<ApiResult<{ id?: string }>>({ action: "addLabor", data });
...
export const payReceivable = (id: string, paid: number) => postJson<ApiResult<Record<string, never>>>({ action: "payReceivable", id, paid });
export const completeReceivable = (id: string) => postJson<ApiResult<Record<string, never>>>({ action: "completeReceivable", id });
