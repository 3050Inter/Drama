...
function summarizeLabor_(rows) { let totalTc = 0, totalAmount = 0; const map = {}; rows.forEach(row => { const pay = parseAmount_(row.dailyPay || row.amount); totalTc += Number(row.tc || 0); totalAmount += pay; if (!map[row.employee]) map[row.employee] = { employee: row.employee, tc: 0, dailyPay: 0, amount: 0 }; map[row.employee].tc += Number(row.tc || 0); map[row.employee].dailyPay += pay; map[row.employee].amount += pay; }); return { totalTc, totalAmount, byEmployee: Object.keys(map).map(key => map[key]) }; }
