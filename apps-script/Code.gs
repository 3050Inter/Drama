...
function readReceivablePayments_() {
  ...
  const values = sh_(SHEETS.receivablePayments).getDataRange().getValues();
  if (values.length < 2) return [];
  return values.slice(1).filter(row => row.some(v => v !== '' && v !== null)).map(row => ({
    date: formatDate_(row[0]),
...
function getReceivableSummary_() { return summarizeReceivables_(readReceivables_()); }
...
function appendReceivableSalesTransaction_(data) {
  ...
  sh_(SHEETS.transactions).appendRow([data.date, '매출', data.method, parseAmount_(data.amount), '', data.memo || '', txId]);
  return txId;
}
