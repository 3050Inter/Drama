...
function ensureLaborHeader_() { ensureHeader_(SHEETS.labor, ['날짜', '직원', '테이블번호', 'TC', '금액', '메모', 'ID', '거래ID']); }
...
function cumulativeCashAll_() { return summarize_(readTransactions_()).cumulativeCash; }
function getHome_(date) {
  ...
  return { ok: true, dashboard: { date: target, ...summarize_(rows), cumulativeCash: cumulativeCashAll_(), receivableBalance: receivable.totalBalance, receivableCount: receivable.count }, rows: rows.slice().reverse() };
}
function getDashboard_(date) { const target = date || todayKst_(); const rows = readTransactions_().filter(r => r.date === target); const receivable = getReceivableSummary_(); return { ok: true, dashboard: { date: target, ...summarize_(rows), cumulativeCash: cumulativeCashAll_(), receivableBalance: receivable.totalBalance, receivableCount: receivable.count } }; }
function getTransactions_(date) { const target = date || todayKst_(); return { ok: true, rows: readTransactions_().filter(r => r.date === target).reverse() }; }
function getMonthly_(month) { const target = month || todayKst_().slice(0, 7); const rows = readTransactions_().filter(r => String(r.date).startsWith(target)); const sum = summarize_(rows); const laborSummary = summarizeLabor_(readLabor_().filter(r => String(r.date).startsWith(target))); return { ok: true, monthly: { month: target, date: '', ...sum, receivableBalance: 0, receivableCount: 0 }, dailySales: [], laborSummary, rows: rows.reverse() }; }
function getStats_(startDate, endDate) {
  ...
  const receivable = getReceivableSummary_();
  return { ok: true, stats: { startDate: start, endDate: end, date: '', ...summarize_(rows), receivableBalance: receivable.totalBalance, receivableCount: receivable.count, personal, rows: rows.reverse(), personalRows: personalRows.reverse() } };
}
...
function getExpenseCategories_() { ensureCategories_(); const values = sh_(SHEETS.categories).getDataRange().getValues(); return { ok: true, rows: values.slice(1).filter(row => String(row[1] || '').toUpperCase() !== 'N').sort((a, b) => Number(a[2] || 0) - Number(b[2] || 0)).map(row => ({ name: String(row[0] || ''), active: String(row[1] || 'Y'), order: Number(row[2] || 0) })) }; }

function getEmployees_() { ensureEmployees_(); const values = sh_(SHEETS.employees).getDataRange().getValues(); return { ok: true, rows: values.slice(1).filter(row => String(row[1] || '').toUpperCase() !== 'N').sort((a, b) => Number(a[2] || 0) - Number(b[2] || 0)).map(row => ({ name: String(row[0] || ''), active: String(row[1] || 'Y'), order: Number(row[2] || 0), id: String(row[3] || '') })) }; }
...
function readLabor_() { ensureLaborHeader_(); const values = sh_(SHEETS.labor).getDataRange().getValues(); if (values.length < 2) return []; return values.slice(1).filter(row => row.some(v => v !== '' && v !== null)).map(row => ({ date: formatDate_(row[0]), employee: String(row[1] || ''), tableNo: String(row[2] || ''), tc: Number(row[3] || 0), amount: parseAmount_(row[4]), memo: String(row[5] || ''), id: String(row[6] || ''), transactionId: String(row[7] || '') })); }
function summarizeLabor_(rows) { let totalTc = 0, totalAmount = 0; const map = {}; rows.forEach(row => { totalTc += Number(row.tc || 0); totalAmount += parseAmount_(row.amount); if (!map[row.employee]) map[row.employee] = { employee: row.employee, tc: 0, amount: 0 }; map[row.employee].tc += Number(row.tc || 0); map[row.employee].amount += parseAmount_(row.amount); }); return { totalTc, totalAmount, byEmployee: Object.keys(map).map(key => map[key]) }; }
function getLabor_(date) { const target = date || todayKst_(); const rows = readLabor_().filter(row => row.date === target).reverse(); return { ok: true, rows, summary: summarizeLabor_(rows) }; }
function addLabor_(data) { ensureLaborHeader_(); const laborId = makeId_('LABOR'); const date = String(data.date || todayKst_()).slice(0, 10); sh_(SHEETS.labor).appendRow([date, String(data.employee || ''), String(data.tableNo || ''), Number(data.tc || 0), 0, String(data.memo || ''), laborId, '']); invalidateDate_(date); return { ok: true, id: laborId }; }
...
function getReceivables_() { const rows = readReceivables_().reverse(); return { ok: true, rows, summary: getReceivableSummary_() }; }
...
function getPersonalExpenses_(date) { const target = date || todayKst_(); const rows = readPersonalExpenses_().filter(row => row.date === target).reverse(); return { ok: true, rows, summary: summarizePersonal_(rows) }; }
